		...
		<aside>
			<div id="sidebar" class="nav-collapse ">
				...
				<ul class="sidebar-menu">
					...
					<li class="active"><a class="" href="keys?type=getkeyforupload"> <span>Upload Files</span>
					</a></li>
...
<!-- 				</div> -->
				<div class="col-md-12">
					<section class="panel">
						...
						<div class="panel-body" style='min-height:600px;'>
							...
							<%
								String keyfile = (String) request.getAttribute("keyfile");
								if (keyfile!=null && keyfile.trim().length()>0) {
									%>
										
										<form action='uploaddata' class='col-md-6'  method = "post" enctype = "multipart/form-data">
											<label>Key File</label>
											<input type=text name='keyfile' value='<%=keyfile%>' readonly="readonly" class='form-control'/>
											<br/>
											<label>Select File to upload on cloud</label>
											<input  required="required" type = "file" name = "file" size = "50" class='form-control' /> 
											<br/>
											<a href='keys?type=getkeyforupload'><< Back</a>
											<input style='float:right;' type=submit value='Upload Data on cloud' class='btn btn-primary' />
										</form>
									...
									%>
											<%
												...
												if (keys==null || keys.size()==0) {
													%>
														<h3>You have to upload your secret key file to proceed</h3>
													<%
												} else {
													%>
														<form action='keys' method=post class='col-md-6'>
															<input type=hidden name='type' value='key_select' />
															<label>Select the secret key for Hybrid Cryptography</label>
															<select class='form-control' name='keyfile'>
															<%
																for (SecretKeys sk : keys) {   
																	%>
																		<option value='<%=sk.getKey_text()%>'><%=sk.getKey_text() %></option>
																	<%
																}
															%>
															</select>
															<br/>
															<input  style='float:right;' type=submit value='Proceed >>' class='btn btn-warning' />
          ...
          -->
					<a href="#">Final Year Project Work</a> &copy; Copyright
				</div>
			</div>
		</section>
		<!--main content end-->
